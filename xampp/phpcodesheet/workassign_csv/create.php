    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['create'])) {
            ...
            if (mysqli_num_rows($checkResult) > 0) {
                echo "<script>alert('Table \"$className\" already exists.');
                ...
                if (mysqli_query($conn, $sql)) {
                    echo "<script>alert('Table \"$className\" created successfully.');
                    location.href = '../../htmlcodesheet/work.php';</script>";
                } else {
            ...
            if (mysqli_num_rows($checkResult) > 0) {
                ...
                if (mysqli_query($conn, $sql)) {
                    echo "<script>alert('Table \"$className\" deleted successfully.'); 
                    location.href = '../../htmlcodesheet/work.php';</script>";
                } else {
                    echo "<script>alert('Error deleting table: " . mysqli_error($conn) . "'); 
                    location.href = '../../htmlcodesheet/work.php';</script>";
                }
            } else {
                echo "<script>alert('Table \"$className\" does not exist.'); 
                location.href = '../../htmlcodesheet/work.php';</script>";
            }
        }
    }
